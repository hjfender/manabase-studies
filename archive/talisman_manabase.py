...
class TalismanManabase():
	...
	def __init__(self, **kwargs):
		...
		self.pab_df = pd.DataFrame(data = {f'Draw {i}' : [self.prob_A_and_B(self.k,self.K+i,self.n1,self.n2,self.n3,self.N)] for i in range(0,7)})
	...
	def prob_A_and_B(self, k, K, n1, n2, n3, N):
		possible_draws = compute_possible_draws(k,[n2,n3],K,N)
		total = 0
		for draw in possible_draws:
